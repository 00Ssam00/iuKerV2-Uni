...
export class CitasMedicasRepositorio implements ICitasMedicasRepositorio {
  ...
  async eliminarCitasPorMedico(tarjetaProfesional: string): Promise<void> {
    await ejecutarConsulta(
      'DELETE FROM historial_paciente WHERE id_cita IN (SELECT id_cita FROM citas_medicas WHERE medico = $1)',
      [tarjetaProfesional]
    );
    await ejecutarConsulta(
  ...
  async eliminarCitasPorPaciente(tipoDocPaciente: number, numeroDocPaciente: string): Promise<void> {
    await ejecutarConsulta(
      'DELETE FROM historial_paciente WHERE id_cita IN (SELECT id_cita FROM citas_medicas WHERE tipo_doc_paciente = $1 AND numero_doc_paciente = $2)',
